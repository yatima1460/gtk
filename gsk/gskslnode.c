...
static gboolean
gsk_sl_node_shift_type_check (GskSlTokenStream *stream,
                              GskSlType        *ltype,
                              GskSlType        *rtype)
{
  GskSlScalarType lscalar, rscalar;
  ...
  if (gsk_sl_type_is_scalar (ltype) && gsk_sl_type_is_vector (rtype))
    {
      if (stream)
        gsk_sl_token_stream_error (stream, "Right operand to shift cannot be a vector if left operand is a scalar.");
      return FALSE;
    }
  if (gsk_sl_type_is_vector (ltype) && gsk_sl_type_is_vector (rtype) &&
      gsk_sl_type_get_length (ltype) != gsk_sl_type_get_length (rtype))
...
static gboolean
gsk_sl_node_relational_type_check (GskSlTokenStream *stream,
                                   GskSlType        *ltype,
                                   GskSlType        *rtype)
{
  if (!gsk_sl_type_is_scalar (ltype))
    {
      if (stream)
        gsk_sl_token_stream_error (stream, "Left operand to relational operator is not a scalar.");
      return NULL;
    }
  if (gsk_sl_type_get_scalar_type (ltype) == GSK_SL_BOOL)
    {
      if (stream)
        gsk_sl_token_stream_error (stream, "Left operand to relational operator must not be bool.");
      return NULL;
    }
  if (!gsk_sl_type_is_scalar (rtype))
    {
      if (stream)
        gsk_sl_token_stream_error (stream, "Right operand to relational operator is not a scalar.");
      return NULL;
    }
  if (gsk_sl_type_get_scalar_type (rtype) == GSK_SL_BOOL)
    {
      if (stream)
        gsk_sl_token_stream_error (stream, "Right operand to relational operator must not be bool.");
      return NULL;
    }
                                      ...
                                      GskSlTokenStream *stream)
{
  ...
  if (!gsk_sl_token_is (token, GSK_SL_TOKEN_IDENTIFIER))
    {
      gsk_sl_token_stream_error (stream, "Expected a function name");
  ...
  if (!gsk_sl_token_is (token, GSK_SL_TOKEN_LEFT_PAREN))
    {
      gsk_sl_token_stream_error (stream, "Expected an openening \"(\"");
  ...
  if (!gsk_sl_token_is (token, GSK_SL_TOKEN_RIGHT_PAREN))
    {
      gsk_sl_token_stream_error (stream, "Expected a closing \")\"");
      gsk_sl_node_unref ((GskSlNode *) function);
      return NULL;
    }
  gsk_sl_token_stream_consume (stream, (GskSlNode *) function);
    ...
    case GSK_SL_TOKEN_IDENTIFIER:
      {
        ...
        if (decl == NULL)
          {
            gsk_sl_token_stream_error (stream, "No variable named \"%s\".", token->str);
        ...
        gsk_sl_token_stream_consume (stream, (GskSlNode *) reference);
                                         ...
                                         GskSlTokenStream *stream)
{
  ...
  gsk_sl_token_stream_consume (stream, (GskSlNode *) assign);
                               ...
                               GskSlTokenStream *stream)
{
  ...
  gsk_sl_token_stream_consume (stream, (GskSlNode *) declaration);
                                       ...
                                       GskSlTokenStream *stream)
{
  ...
  if (!gsk_sl_token_is (token, GSK_SL_TOKEN_LEFT_BRACE))
    {
      gsk_sl_token_stream_error (stream, "Expected an opening \"{\"");
      gsk_sl_node_unref ((GskSlNode *) function);
      return NULL;
    }
  gsk_sl_token_stream_consume (stream, (GskSlNode *) function);
  ...
  while (TRUE)
    {
      ...
      case GSK_SL_TOKEN_EOF:
        gsk_sl_token_stream_error (stream, "Unexpected end of function, expected \"}\"");
        goto out;
                           ...
                           GskSlTokenStream *stream)
{
  ...
  for (token = gsk_sl_token_stream_get (stream);
                         ...
                         GError **error)
{
  ...
  stream = gsk_sl_token_stream_new (source);

  gsk_sl_node_parse_program (program, stream);

  gsk_sl_token_stream_unref (stream);
